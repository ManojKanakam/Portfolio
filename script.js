...
window.addEventListener("scroll", () => {
  ...
  sections.forEach((section) => {
    ...
    // Adjust offset to account for fixed header, or make it dynamic if header height changes
    const sectionHeight = section.clientHeight
    if (scrollY >= sectionTop - 200) { // Adjusted offset for better active state
...
// General Animations (Intersection Observer)
...
window.addEventListener("load", () => {
  ...
  // Slide in animations (timeline items)
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  ) // Targeting specifically the phone number span in contact
  ...
  const emailElements = document.querySelectorAll(".contact-item:nth-child(2) span") // Targeting specifically the email span in contact
...
// Preloader Styling (Dynamically added for demonstration)
window.addEventListener("load", () => {
